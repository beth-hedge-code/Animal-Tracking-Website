...
// Get vet info for an animal by animalID, with optional filters
router.get("/api/vet/:animalid", (req, res) => {
  const animalID = req.params.animalid;

  // Extract filter params from query string
  const { dateFrom, dateTo, minWeight, maxWeight, sortBy, order } = req.query;
  ...
  // Sorting
  const validSort = ["visitDate", "weight", "vetName"];
  const sortColumn = validSort.includes(sortBy) ? sortBy : "visitDate";
  const sortOrder = order && order.toLowerCase() === "asc" ? "ASC" : "DESC";
  sql += ` ORDER BY vv.${sortColumn} ${sortOrder}`;
